...
import ru.donny.burnmeter3D.engine.burnprocessors.BurnHealer;
...
public class ModelScreen extends StandartScreen {
    ...
    protected MaterialUi ui;

    ModelInstance renderMe;
    ...
    private void initUi() {
        ui = new MaterialUi();
        ui.getRootTable().debug();
        ui.addIcon("play.png", new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                if (modelView.getBurnsStorage().asArray().size() > 0) {
                    showCenter();
                }
            }
        }).top().expandX().right();

        mainStage.addActor(ui.getRootTable());
    }

    void showCenter() {
        BurnHealer healer = new BurnHealer(modelView.getBurnsStorage().asArray().get(modelView.getBurnsStorage().asArray().size() - 1),
                modelAbstraction.getGraph(), modelView.getBurnsStorage().asArray());
        healer.makeStep();
//        createTriangleDebug(healer.getCenter());
    }

    void createTriangleDebug(Triangle triangle) {
        renderMe = BurnModelBuilder.build(new ArrayList<Triangle>(Arrays.asList(triangle)), Burn.BurnType.III);
    }
    ...
    private void initInterface() {
        ...
        mainStage.addActor(userInterface);
    }
...
//		cameraControl.prepareCameraAsync();
    ...
    public void render(float delta) {
        ...
        if (renderMe != null)
            modelBatch.render(renderMe, modelView.getEnvironment());
